  ...
  <table class="table-fixed w-full">
      <thead>
          <tr class="bg-gray-100">
              <th class="px-4 py-2 w-15"><a wire:click.prevent="sortBy('id')" role="button" href="#">
                Id
              ...
              <th class="px-4 py-2"><a wire:click.prevent="sortBy('date_of_birth')" href="#">
                DOB
                @include('includes._sort_icon', ['field' => 'date_of_birth'])
              </a></th>
              <th class="px-4 py-2"><a wire:click.prevent="sortBy('hire_date')" href="#">
                Hire Date
      ...
      <tbody>
        @foreach ($employees as $employee)
          <tr>
              ...
              <td class="border px-4 py-1">{{ $employee->first_name }}</td>
              <td class="border px-4 py-1">{{ $employee->last_name }}</td>
              <td class="border px-4 py-1">{{ $employee->date_of_birth->format('m-d-Y') }}</td>
              <td class="border px-4 py-1">{{ $employee->hire_date->format('m-d-Y') }}</td>
